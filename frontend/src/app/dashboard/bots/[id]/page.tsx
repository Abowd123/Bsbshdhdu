...
export default function BotDetailPage() {
  ...
  }, [tab, botId, currentPath]);

  useEffect(() => {
    if (tab !== "stats" || bot?.status !== "running") return;

    const fetchStats = () => {
      apiClient
        .get(`/bots/${botId}/stats`)
        .then((res) => {
          const data: BotStats = res.data;
          setStats(data);
          setStatsHistory((prev) => {
            const next = [
              ...prev,
              {
                ...
                ram: data.memory_usage_mb,
              },
            ];
            return next.slice(-30); // آخر 30 قراءة فقط (~5 دقائق)
          });
        })
        .catch(() => setStats(null));
    };
          ...
          {stats ? (
            <>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <Card><p className="text-slate-500 text-xs mb-1">Processes</p><p className="text-2xl font-display font-bold text-slate-100">{stats.process_count}</p></Card>
              ...
              <Card>
                <p className="text-slate-500 text-xs mb-3">CPU % (آخر 5 دقائق تقريبًا)</p>
                <ResponsiveContainer width="100%" height={160}>
                  <LineChart data={statsHistory}>
                    <XAxis dataKey="time" hide />
                    <YAxis width={32} tick={{ fontSize: 10, fill: "#64748b" }} />
                    <Tooltip
              ...
              <Card>
                <p className="text-slate-500 text-xs mb-3">Memory MB (آخر 5 دقائق تقريبًا)</p>
                <ResponsiveContainer width="100%" height={160}>
                  <LineChart data={statsHistory}>
                    <XAxis dataKey="time" hide />
                    <YAxis width={32} tick={{ fontSize: 10, fill: "#64748b" }} />
                    <Tooltip
                      ...
                      labelStyle={{ color: "#94a3b8" }}
                    />
                    <Line type="monotone" dataKey="ram" name="RAM MB" stroke="#38bdf8" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </Card>
            </>
          ) : (
            <p className="text-slate-500 text-sm">شغّل البوت لعرض الإحصائيات المباشرة</p>
          )}
        </div>
      )}
    </div>
