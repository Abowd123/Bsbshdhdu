...
class FileManager:
    ...
    def resolve_for_download(self, relative_path: str) -> Path:
        """يتحقق من المسار (مع الحماية من path traversal) ويعيد المسار المطلق
        لملف موجود وقابل للتحميل كما هو (بدون قراءته كنص، يدعم أي نوع ملف)."""
        target = self._resolve(relative_path)
        if not target.exists() or target.is_dir():
            raise FileManagerError("file_not_found")
        return target
