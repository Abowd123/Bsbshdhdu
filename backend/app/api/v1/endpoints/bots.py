...
async def get_stats(bot_id: uuid.UUID, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    bot = await _get_bot_or_404(db, user, bot_id)
    stats = await monitoring_service.get_latest(str(bot_id))
    if stats is None:
        # لا توجد قراءة مخزّنة بعد في Redis (مثلاً أول تشغيل) — نجلب لقطة فورية ونخزّنها
        stats = await monitoring_service.collect_and_store(str(bot_id), bot.container_name)
    if not stats:
...
@router.get("/{bot_id}/files/download")
async def download_file(bot_id: uuid.UUID, path: str, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    bot = await _get_bot_or_404(db, user, bot_id)
    fm = FileManager(bot.storage_path)
    try:
        target = fm.resolve_for_download(path)
    except FileManagerError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
...
@router.post("/{bot_id}/files/copy")
async def copy_file(bot_id: uuid.UUID, path: str, payload: FileMoveRequest, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    bot = await _get_bot_or_404(db, user, bot_id)
    fm = FileManager(bot.storage_path)
    try:
        fm.copy(path, payload.destination)
    except FileManagerError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
